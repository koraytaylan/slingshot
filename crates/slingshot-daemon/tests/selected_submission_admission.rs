...
#[test]
fn runtime_builder_reconciles_maintenance_producers_without_operation_owners() {
    ...
    use slingshot_domain::daemon_runtime_contract::{DaemonRuntimeContract, MaintenanceResultKind};
    use slingshot_local_protocol::foundation_contract::FoundationContract;
    use slingshot_storage::{
        database::RequiredSettings, persistent_capacity::MaintenancePublicationRecovery,
    };
    for mode in ["staged", "published", "associated", "foreign"] {
        ...
        let build = || {
            let Acquisition::Owned(owner) =
                ...
                panic!("namespace must be released")
            };
        ...
        let target = if mode == "foreign" {
            "a".repeat(DIGEST_HEX_CHARACTERS)
        } else {
            runtime.context().target().author_target_identity_digest.clone()
        };
        let source = "b".repeat(DIGEST_HEX_CHARACTERS);
        let account = runtime.capacity();
        ...
        if mode == "associated" {
            slingshot_storage::maintenance_results::record_current_preview(
                runtime.database(),
                &account,
                &target,
                &source,
        ...
        let resumed = build();
        if mode == "foreign" {
            assert_eq!(resumed.unwrap_err(), RuntimeBuildRefusal::Resources);
            continue;
        }
        let resumed = resumed.unwrap();
        assert_eq!(
            resumed.maintenance_publication_recovery(),
            Some(if mode == "associated" {
                MaintenancePublicationRecovery::Completed
            } else {
                MaintenancePublicationRecovery::Abandoned
            })
        );
        assert!(resumed.pending_publications().is_empty());
        assert_eq!(resumed.capacity().pending_publications().unwrap(), 0);
        assert_eq!(resumed.capacity().usage().unwrap().operation_rows, 0);
        assert_eq!(
            slingshot_storage::maintenance_results::read(
                resumed.database(),
                &target,
                &document.identifier
            )
        ...
        drop(resumed);
        assert_eq!(build().unwrap().maintenance_publication_recovery(), None);
    }
}
...
async fn runtime_concrete_executor_preserves_paused_recovery_and_shutdown() {
    ...
    use slingshot_daemon::{
        ownership::{Acquisition, DaemonOwnership},
        runtime_builder::{RuntimeBuilder, RuntimeExecutionRefusal},
        runtime_namespace::RuntimeNamespace,
    };
    use slingshot_domain::{
        ...
        operation_executor::{ExecutionIdentity, OperationExecutorOutcome, ProgressPort},
    };
    use slingshot_local_protocol::foundation_contract::FoundationContract;
    struct Progress;
    ...
    let contract = FoundationContract::embedded();
    let namespace =
        RuntimeNamespace::name(&contract, &root.path().join("runtime"), "remote-site", "staging")
            .unwrap();
    namespace.create_runtime_directory().unwrap();
    let Acquisition::Owned(owner) = DaemonOwnership::acquire(&contract, namespace.clone()).unwrap()
    else {
    ...
    let identity = ExecutionIdentity {
        ...
        operation_identifier: "paused".into(),
    };
    ...
    .unwrap();
    let fingerprint = slingshot_domain::command_fingerprint::CommandFingerprint::derive(
        &slingshot_domain::command_fingerprint::FingerprintInput {
    ...
    runtime
        ...
        .admit(
            &slingshot_storage::operation_repository::AdmissionRequest {
                author_target_identity: "retained-identity".into(),
                author_target_identity_digest: identity.author_target_identity_digest.clone(),
    ...
    let recovery = RecoveryFact {
        attempt_count:
            slingshot_daemon::operation::recovery_and_event_supervisor::automatic_attempt_cap()
                as u32,
        category: RecoveryCategory::OperationLookup,
        detail: "persisted pause".into(),
    ...
    let before = runtime
        ...
        .unwrap();
    assert_eq!(
        runtime.execute_retained(&identity, submission.clone(), &Progress).await.unwrap(),
    ...
    foreign.selected_environment_revision = "foreign".into();
    assert_eq!(
        runtime.execute_retained(&foreign, submission.clone(), &Progress).await.unwrap_err(),
        RuntimeExecutionRefusal::Binding
    );
    runtime.request_shutdown();
    assert_eq!(
        runtime.execute_retained(&identity, submission, &Progress).await.unwrap_err(),
        RuntimeExecutionRefusal::Cancelled
    );
...
fn runtime_builder_establishes_and_reopens_only_ledger_bound_state() {
    ...
    for defect in [
        ...
        if defect == "none" {
            ...
            let approved =
                slingshot_storage::maintenance::preview(durable.database(), target, 1000, 1)
                    .unwrap();
            ...
            durable
                ...
                .admit(
                    &slingshot_storage::operation_repository::AdmissionRequest {
                        author_target_identity: "retained-test-identity".into(),
                        author_target_identity_digest: target.clone(),
                        caller_identity: Some("caller".into()),
                        canonical_command: canonical.into(),
                        command_fingerprint: fingerprint,
                        command_wire_name: "query_paths".into(),
                        daemon_runtime_contract_digest: durable
                            .context()
                            .target()
                            .daemon_runtime_contract_digest
                            .clone(),
                        installation_identifier: identity.clone(),
                        operation_identifier: "retained-local".into(),
                        selected_environment_revision: revision.clone(),
                        workflow_correlation_identifier: None,
                    },
                    1000,
        ...
        assert_eq!(
            ledger.read().unwrap().registration(&namespace.key()),
            Some(TargetRegistration::Registered)
        );
        assert!(!namespace.readiness_path().exists());
        assert!(matches!(
            DaemonOwnership::acquire(&contract, namespace.clone()).unwrap(),
            Acquisition::AlreadyOwned(_)
        ));
        drop(durable);
        ...
        match defect {
            ...
            "foreign-identity" => {
                record.installation_identifier =
                    InstallationIdentifier::parse(&"f".repeat(DIGEST_HEX_CHARACTERS)).unwrap();
            ...
            "staged-existing" | "staged-absent" => {
                ...
                ledger.replace(&record).unwrap();
                if defect == "staged-absent" {
                    std::fs::remove_file(paths.database_path()).unwrap();
                }
            }
            ...
            "abandoned-stage" => {
                ...
                let path = paths
                    ...
                    .join(format!("6ca0aa08-3d39-4b18-bfbe-f2bd859947a0{STAGING_SUFFIX}"));
        ...
        if matches!(defect, "none" | "staged-existing" | "staged-absent" | "abandoned-stage") {
            ...
            if defect == "none" {
                ...
                assert_eq!(
                    resumed.maintenance_recovery()[0].stage,
                    slingshot_storage::maintenance::ReceiptStage::Completed
                );
                ...
                assert_eq!(
                    recovered.input.summary.record.lifecycle_state,
                ...
                assert_eq!(
                    publication.artifact_slot,
                    slingshot_storage::artifact_store::STRUCTURED_RESULT_SLOT
                );
            ...
            let mut service =
                slingshot_daemon::service::DaemonService::from_runtime(contract.clone(), resumed);
            ...
            assert_eq!(
                ...
                selected.daemon_runtime_contract_digest
            );
            assert_eq!(published.retained_control_version, contract.control.version);
            assert_eq!(
                published.supported_operation_versions,
                vec![
                    slingshot_domain::daemon_runtime_contract::DaemonRuntimeContract::embedded()
            ...
            let operation_request = serde_json::json!({
                "author_target_identity_digest": selected.author_target_identity_digest,
                "daemon_runtime_contract_digest": selected.daemon_runtime_contract_digest,
                "operation_protocol_version": slingshot_domain::daemon_runtime_contract::DaemonRuntimeContract::embedded().operation_protocol_version,
                "request_identifier": "runtime-status",
                "request": { "request": "operation_status", "operation_identifier": "missing-operation" },
                "selected_environment_revision": selected.selected_environment_revision,
            });
            let served = service.answer(&serde_json::to_vec(&operation_request).unwrap());
            let payload = slingshot_local_protocol::framing::read_payload(
                &contract.framing,
                &served.frame()[4..],
            )
            .unwrap();
            let response: slingshot_local_protocol::message::OperationResponse =
                serde_json::from_str(payload).unwrap();
            assert!(matches!(
                response,
                slingshot_local_protocol::message::OperationResponse::MissingOperation { .. }
            ));
            let frame = slingshot_local_protocol::framing::render(
                &contract.framing,
                &serde_json::to_vec(&operation_request).unwrap(),
            )
            .unwrap();
            ...
            runtime.block_on(async {
                ...
                let (mut client, mut server) = tokio::io::duplex(
                    usize::try_from(contract.framing.maximum_payload_bytes).unwrap_or(usize::MAX),
                );
                let client_task = async {
                    ...
                    let payload = slingshot_local_protocol::framing::read_payload(&contract.framing, &body).unwrap();
                    let response: slingshot_local_protocol::message::OperationResponse = serde_json::from_str(payload).unwrap();
                    assert!(matches!(response, slingshot_local_protocol::message::OperationResponse::MissingOperation { .. }));
                    drop(client);
                };
                let server_task = slingshot_daemon::local_server::serve_connection(&service, &mut server);
                let (_, served) = tokio::join!(client_task, server_task);
            ...
            assert!(
                ...
                "service creator released a live connection's runtime"
            );
            assert!(matches!(
                DaemonOwnership::acquire(&contract, namespace.clone()).unwrap(),
                Acquisition::AlreadyOwned(_)
            ...
            assert!(matches!(
                result.unwrap_err(),
                RuntimeBuildRefusal::Installation
                    | RuntimeBuildRefusal::Database
                    | RuntimeBuildRefusal::Resources
            ));
...
async fn runtime_builder_binds_snapshot_names_and_holds_ownership_without_readiness() {
    ...
    for (profile, environment, accepted) in [
        ("remote-site", "staging", true),
        ("other-site", "staging", false),
        ("remote-site", "production", false),
    ] {
        let directory = tempfile::tempdir().unwrap();
        ...
        let contract = FoundationContract::embedded();
        let namespace =
            RuntimeNamespace::name(&contract, &runtime_root, profile, environment).unwrap();
        namespace.create_runtime_directory().unwrap();
        let Acquisition::Owned(owner) =
            DaemonOwnership::acquire(&contract, namespace.clone()).unwrap()
        else {
            panic!("fresh namespace owned")
        };
        ...
        if accepted {
            ...
            assert_eq!(
                builder.target().daemon_runtime_contract_digest,
...
async fn selected_live_events_commit_only_the_believed_prefix() {
    use slingshot_agent_connection::{
        ...
        server_sent_event_decoder::{OperationStreamExpectation, StreamItem},
    };
    use slingshot_agent_protocol::{
        identity::WireOperationIdentity, wire_contract::ExpectedProvenance,
    };
    use slingshot_daemon::operation::{
        durable_author_event::{DurableEventOutcome, fold_selected_event},
        durable_author_submission::prepare_initial_submission,
    };
    use slingshot_daemon::operation::{
        selected_event_attachment::{
            SelectedEventAttachmentOutcome,
            attach_selected_events_with_authentication as attach_selected_events,
        },
    ...
    for owned in [false, true] {
        for mode in 0..5 {
            ...
            let selected_protocol = match mode {
                0 => ResetTransport::Http1,
                1 => ResetTransport::Http2,
                _ => ResetTransport::Automatic,
            };
            for defect in [
                "",
                "gap",
                "regression",
                "terminal",
                "terminal-no-result-repeat",
                "terminal-no-result-cap",
                "terminal-capability-error",
                "terminal-lookup-error",
                "terminal-lookup-error-cap",
                "terminal-lookup-error-success",
                "terminal-wait-cancel",
                "terminal-complete",
                ...
                "job-conflict",
                "cursor-gap-conflict",
                "cursor-stale-conflict",
                "terminal-foreign-auth",
            ] {
                if defect == "terminal-foreign-auth" && mode >= 3 {
                    continue;
                }
                ...
                let (authentication, _) =
                    provider.authenticate(&endpoint, 1, &NoTokenSource).unwrap();
                let identity = ExecutionIdentity {
                    attempt: 1,
                    operation_identifier: "event-local".into(),
                ...
                let ledger = AgentSubscriptionLedger::new(
                ...
                installation_identifier: InstallationIdentifier::parse(&"a1".repeat(DIGEST_BYTE_PAIRS)).unwrap(), operation_identifier: identity.operation_identifier.clone(),
                ...
                .unwrap();
                drop(
                    prepare_initial_submission(&repository, &identity, &submission, 1000)
                        .unwrap()
                        .unwrap(),
                );
                ...
                .contains(&defect)
                {
                    ...
                    let known_success = defect == "terminal-lookup-error-success"
                        || defect.starts_with("terminal-no-result");
                    if known_success {
                        repository
                            .fold_event(
                                &before.identity,
                                before.observation.applied_sequence,
                                slingshot_domain::remote_job::RemoteJobObservation {
                                    applied_sequence:
                                        slingshot_domain::remote_job::JobEventSequence::of(
                                            first_sequence,
                                        ),
                                    state: slingshot_domain::remote_job::AgentJobState::Running,
                                    attempt: 2,
                                    progress: 40,
                                },
                            )
                            .unwrap();
                        repository
                            .record_physical_job(&before.identity, "event-job", 1000)
                            .unwrap();
                    }
                ...
                second["sequence"] = (first_sequence + 1).into();
                second.as_object_mut().unwrap().remove("attempt");
                second.as_object_mut().unwrap().remove("progress");
                match defect {
                    "gap" => second["sequence"] = (first_sequence + 2).into(),
                    "regression" => second["progress"] = 0.into(),
                    name if name.starts_with("terminal") => {
                        let state =
                            if name == "terminal-complete-failed" { "failed" } else { "succeeded" };
                        second["kind"] = state.into();
                        second["state"] = state.into();
                        second["terminal"] = serde_json::json!({"provenance":submission.provenance, "submitted_command_digest":submission.submitted_command_digest});
                    }
                    "unknown" => {
                        second["agent_operation_identifier"] =
                            "b".repeat(DIGEST_HEX_CHARACTERS).into()
                    }
                    "replay" => second["sequence"] = first_sequence.into(),
                    "stale" => second["sequence"] = (first_sequence - 1).into(),
                    "job-conflict" => {
                        second["sequence"] = first_sequence.into();
                        second["progress"] = 41.into();
                    }
                ...
                let body = format!(
                    ...
                    if defect == "cursor" {
                        ""
                    } else if defect.starts_with("cursor-") {
                        "id:cursor-001\n"
                    } else {
                        "id:cursor-002\n"
                    }
                ...
                let peer = async {
                    ...
                    if owned {
                        assert!(
                            request
                                .windows(b"cursor-000".len())
                                .any(|bytes| bytes == b"cursor-000")
                        );
                    }
                    if http2 {
                        ...
                        for (kind, flags, bytes) in
                            [(1, 4, block.as_slice()), (0, 1, body.as_bytes())]
                        {
                ...
                let selected_authentication = if mode == 4 {
                    AuthorAuthentication::AsyncProvider {
                        provider: &async_provider,
                        clock: &NoTokenClocks,
                        utc: &NoTokenClocks,
                    }
                } else if mode == 3 {
                    AuthorAuthentication::Provider {
                        provider: &provider,
                ...
                let resolver = |operation: &str| {
                    Ok(OperationStreamExpectation {
                        daemon_subscription_identifier: "subscription-one".into(),
                        agent_event_store_generation: 7,
                        agent_operation_identifier: operation.into(),
                        expected_provenance: expected.clone(),
                    ...
                    if owned {
                        ...
                        .await
                        {
                            ...
                            Ok(SelectedEventAttachmentOutcome::TerminalRecovery(ticket)) => {
                                assert_eq!(
                                    ticket.agent_operation_identifier(),
                                    submission.operation.agent_operation_identifier
                                );
                                terminal_ticket = Some(ticket);
                                owned_reason = Some(DurableEventOutcome::NeedsTerminalLookup);
                                Err(FiniteHttpFailure::Body)
                            }
                            Ok(SelectedEventAttachmentOutcome::Recovery {
                                agent_operation_identifier,
                                reason,
                            }) => {
                                assert_eq!(
                                    agent_operation_identifier.as_deref(),
                                    Some(submission.operation.agent_operation_identifier.as_str())
                                );
                                owned_reason = Some(reason);
                                Err(FiniteHttpFailure::Body)
                            }
                ...
                let (_, result) =
                    timeout(Duration::from_secs(5), async { tokio::join!(peer, request) })
                        .await
                        .unwrap();
                if let Some(reason) = owned_reason {
                    outcomes.push(reason);
                }
                let success = ["", "unknown", "replay", "stale"].contains(&defect);
                assert_eq!(result.is_ok(), success, "{owned}/{http2}/{defect}: {result:?}");
                if !owned {
                    assert_eq!(heartbeat, success);
                }
                let after = repository
                    ...
                    .unwrap();
                assert_eq!(
                    after.observation.applied_sequence.value(),
                    first_sequence + u64::from(defect.is_empty())
                );
                assert_eq!((after.observation.attempt, after.observation.progress), (2, 40));
                assert_eq!(
                    after.remaining_retention_milliseconds,
                    before.remaining_retention_milliseconds
                );
                assert_eq!(
                    after.request_start_unix_milliseconds,
                    before.request_start_unix_milliseconds
                );
                assert_eq!(
                    repository
                        .physical_jobs(target, &submission.operation.agent_operation_identifier)
                        .unwrap(),
                    ["event-job"]
                );
                let held = ledger.read_subscription(target, "subscription-one").unwrap().unwrap();
                assert_eq!(
                    held.cursor.as_deref(),
                    Some(if success { "cursor-002" } else { "cursor-001" })
                );
                assert_eq!(held.event_rows, if success { 2 } else { 1 });
                if defect == "terminal" {
                    assert_eq!(outcomes.last(), Some(&DurableEventOutcome::NeedsTerminalLookup));
                }
                if let Some(ticket) = terminal_ticket {
                    assert_eq!(format!("{ticket:?}"), "CapturedTerminalEvent([redacted])");
                    if defect == "terminal-foreign-auth" {
                        let foreign = EnvironmentAuthenticationProvider::new(
                            selected_snapshot_with_publisher(
                                &endpoint,
                                "http://another-publisher.example.com",
                            ),
                            2,
                        );
                        ...
                        let (foreign_fixed, _) =
                            foreign.authenticate(&endpoint, 0, &NoTokenSource).unwrap();
                        let local_before =
                            operations.read(target, &identity.operation_identifier).unwrap();
                        let remote_before = repository
                            .read(target, &submission.operation.agent_operation_identifier)
                            .unwrap();
                        assert!(
                            ticket
                                .reconcile(&repository, &transport, &foreign_fixed, None)
                                .await
                                .is_err()
                        );
                        assert_eq!(
                        ...
                        operations
                            .apply(
                                ...
                                &slingshot_domain::operation::OperationFact::Progress {
                                    detail: "new owner observation".into(),
                                },
                                1002,
                            )
                            .unwrap();
                    ...
                    if defect == "terminal-physical-race" {
                        repository
                            .record_physical_job(&before.identity, "concurrent-job", 1002)
                            .unwrap();
                    }
                    let local_before =
                    ...
                    let remote_before = repository
                        ...
                        .unwrap()
                        .unwrap();
                    let other = AgentJobRepository::new(
                        OperationDatabase::open(
                            &root.path().join("other-owner.sqlite3"),
                            settings(),
                        )
                        .unwrap(),
                    );
                    ...
                    let peer = async {
                        if ["terminal-local-race", "terminal-physical-race", "terminal-owner"]
                            .contains(&defect)
                        {
                            return;
                        }
                        for (route, document) in [
                            ...
                            assert!(
                                request.windows(route.len()).any(|bytes| bytes == route.as_bytes())
                            );
                            authentication.lend_value_bytes(|value| {
                                assert!(request.windows(value.len()).any(|bytes| bytes == value))
                            });
                            if http2 {
                                    ...
                                    [(1, 4, block.as_slice()), (0, 1, body.as_bytes())]
                                {
                                    ...
                                    socket
                                        .write_all(&[
                                            length[1], length[2], length[3], kind, flags, 0, 0, 0,
                                            1,
                                        ])
                                        .await
                                        .unwrap();
                    ...
                    let (recovered, ()) = if defect == "terminal-wait-cancel" {
                        tokio::time::pause();
                        let mut waiting =
                            Box::pin(ticket.reconcile_saved(&repository, &transport, None));
                        ...
                        timeout(Duration::from_secs(5), async {
                            tokio::join!(
                                ticket.reconcile_saved(
                                    if defect == "terminal-owner" { &other } else { &repository },
                                    &transport,
                                    None
                                ),
                                peer
                    ...
                    if defect == "terminal" || defect.starts_with("terminal-no-result") {
                        ...
                        assert_eq!(
                            retry.category,
                            slingshot_domain::operation::RecoveryCategory::ResultAcquisition
                        );
                        assert_eq!(
                            retry.attempt_count,
                            local_before
                                ...
                                .as_ref()
                                .map_or(1, |previous| previous.attempt_count + 1)
                        );
                        assert_eq!(
                            retry.manual_resume_eligible,
                            defect == "terminal-no-result-cap"
                        );
                        if retry.manual_resume_eligible {
                            assert_eq!(retry.retry_delay_milliseconds, 0);
                        } else {
                        ...
                        if defect == "terminal-no-result-cap" {
                            ...
                            for (now, attempt) in [(1000, identity.attempt), (9_000_000, 99)] {
                                ...
                                assert_eq!(
                                    ledger
                                        .read_subscription(target, "subscription-one")
                                        .unwrap()
                                        .unwrap(),
                                    held
                                );
                                assert!(
                                    timeout(Duration::from_millis(10), listener.accept())
                                        .await
                                        .is_err(),
                                    "paused concrete adapter opened a socket"
                                );
                            }
                        }
                        ...
                        if defect == "terminal-complete" {
                            assert_eq!(
                                local_after.record.lifecycle_state,
                            ...
                            assert_eq!(
                                local_after.record.terminal_failure.as_ref().unwrap().kind,
                                slingshot_domain::operation::TerminalFailureKind::Rejected
                            );
                        ...
                        assert!(
                            ledger
                                .read_recovery_view(target, "subscription-one")
                                .unwrap()
                                .members()
                                .is_empty()
                        );
                        for conflict in [false, true] {
                            let replay_ledger = AgentSubscriptionLedger::new(
                                OperationDatabase::open(&path, settings()).unwrap(),
                            );
                            let replay_operations = OperationRepository::new(
                                OperationDatabase::open(&path, settings()).unwrap(),
                            );
                            let mut replay = second.clone();
                            if conflict {
                                replay["progress"] = 41.into();
                            }
                            ...
                            let peer = async {
                                ...
                                assert!(
                                    request
                                        ...
                                        .any(|bytes| bytes == previous.as_bytes())
                                );
                                authentication.lend_value_bytes(|value| {
                                    assert!(
                                ...
                                if http2 {
                                        ...
                                        [(1, 4, block.as_slice()), (0, 1, body.as_bytes())]
                                    {
                                        ...
                                        socket
                                            .write_all(&[
                                                length[1], length[2], length[3], kind, flags, 0, 0,
                                                0, 1,
                                            ])
                                            .await
                                            .unwrap();
                            ...
                            if conflict {
                                ...
                                assert!(matches!(
                                    outcome,
                                    SelectedEventAttachmentOutcome::Transport(_)
                                ));
                            }
                            assert_eq!(
                                repository
                            ...
                            assert_eq!(
                                replayed.unresolved_incident.as_deref(),
                                if conflict { Some("cursor-003") } else { None }
                            );
                        }
                    } else if [
                        "terminal-capability-error",
                    ...
                    .contains(&defect)
                        || defect.starts_with("terminal-lookup-error")
                    {
                        let retry = local_after.record.outstanding_recovery.as_ref().unwrap();
                        ...
                        assert_eq!(
                            ...
                            local_before
                                ...
                                .outstanding_recovery
                                .as_ref()
                                .map_or(1, |held| held.attempt_count + 1)
                        );
                        ...
                        assert_eq!(
                            retry.category,
                            if defect == "terminal-lookup-error-success" {
                                slingshot_domain::operation::RecoveryCategory::ResultAcquisition
                            } else {
                                slingshot_domain::operation::RecoveryCategory::OperationLookup
                            }
                        );
                        assert!(retry.retry_delay_milliseconds<=slingshot_daemon::operation::recovery_and_event_supervisor::jitter_ceiling_milliseconds(u64::from(retry.attempt_count)));
                        assert_eq!(
                            retry.manual_resume_eligible,
                            defect == "terminal-lookup-error-cap"
                        );
                        if retry.manual_resume_eligible {
                            assert_eq!(retry.retry_delay_milliseconds, 0);
                        }
                ...
                if defect == "gap" {
                    assert_eq!(outcomes.last(), Some(&DurableEventOutcome::NeedsSnapshot));
                }
                if defect.ends_with("conflict") {
                    assert_eq!(outcomes.last(), Some(&DurableEventOutcome::NeedsIntegrityRecovery));
                    let reopened = AgentSubscriptionLedger::new(
                        OperationDatabase::open(&path, settings()).unwrap(),
                    );
                    let view = reopened.read_recovery_view(target, "subscription-one").unwrap();
                    assert_eq!(
                        view.ledger().unresolved_incident.as_deref(),
                        Some(if defect == "job-conflict" { "cursor-002" } else { "cursor-001" })
                    );
                    assert_eq!(view.ledger().unresolved_incident_count, 1);
                    reopened.record_event_conflict(&view, "cursor-999").unwrap();
                    assert_eq!(
                        reopened.read_subscription(target, "subscription-one").unwrap().unwrap(),
                        *view.ledger()
                    );
                    if owned {
                        let stopped = attach_selected_events(
                            &reopened,
                            &operations,
                            &transport,
                            &identity,
                        ...
                        if mode == 4 && defect == "job-conflict" {
                            for revision_only in [false, true] {
                                let foreign_endpoint = if revision_only {
                                    endpoint.clone()
                                } else {
                                    format!("{endpoint}/foreign")
                                };
                                let publisher = if revision_only {
                                    "http://another-publisher.example.com"
                                } else {
                                    "http://publish.example.com"
                                };
                                let foreign = EnvironmentAuthenticationProvider::new(
                                    selected_snapshot_with_publisher(&foreign_endpoint, publisher),
                                    2,
                                );
                                ...
                                assert_eq!(
                                    foreign.snapshot().target() == provider.snapshot().target(),
                                    revision_only
                                );
                                assert_ne!(
                                    foreign.snapshot().revision(),
                                    provider.snapshot().revision()
                                );
                                let (foreign_fixed, _) = foreign
                                    .authenticate(&foreign_endpoint, 0, &NoTokenSource)
                                    .unwrap();
                                for policy in [
                                    AuthorAuthentication::Fixed {
                                        authentication: &foreign_fixed,
                                        protocol: ResetTransport::Automatic,
                                    },
                                    AuthorAuthentication::Provider {
                                        provider: &foreign,
                                        source: &NoTokenSource,
                                    ...
                                    assert!(
                                        ...
                                        .await
                                        .is_err(),
                                        "foreign policy returned retained recovery"
                                    );
                                    assert_eq!(
                                        reopened
                                            .read_subscription(target, "subscription-one")
                    ...
                    assert_eq!(held.unresolved_incident, None);
                }
                if owned && defect.is_empty() {
                    rusqlite::Connection::open(&path)
                        .unwrap()
                        .execute("UPDATE agent_operation SET argument_schema_digest = 'drift'", [])
                        .unwrap();
                    assert!(
                        attach_selected_events(
                            &ledger,
...
async fn subscription_reset_stages_two_authenticated_snapshots_before_atomic_installation() {
    ...
    use slingshot_daemon::operation::{
        durable_author_submission::prepare_initial_submission,
        subscription_reset::{ResetTransport, SubscriptionResetOutcome, reset_active_subscription},
    };
    use slingshot_domain::{
        agent_identity::AgentEventStoreGeneration,
        command_fingerprint::{CommandFingerprint, FingerprintInput},
    ...
    use slingshot_storage::{
        agent_job_repository::AgentJobRepository,
        agent_subscription_ledger::{AgentSubscriptionLedger, EventFact},
        database::{OperationDatabase, RequiredSettings},
        operation_repository::{AdmissionRequest, OperationRepository},
    };
    use tokio::{
    ...
    for mode in 0..5 {
        ...
        for defect in [
            ...
            "paused-exhausted",
            "paused-capacity",
            "paused-after-capture",
            "eligible-not-paused",
            "generation-missing",
            "generation-unanswered",
            ...
            "generation-held-conflict",
            "generation-held-older",
            "generation-settle-owner",
            "generation-settle-stale",
            "generation-settle-terminal",
            "generation-settle-known-success",
            "generation-finish",
            "generation-already-settled",
            "generation-unanswered-cap",
            "generation-unanswered-success",
            "generation-unanswered-stale",
            "generation-unanswered-dispatch",
            ...
            let selection = ExecutionIdentity {
                attempt: 1,
                operation_identifier: "local-0".into(),
                author_target_identity_digest: provider.snapshot().target().to_string(),
                selected_environment_revision: provider.snapshot().revision().to_string(),
            };
            let expected = ExpectedProvenance {
            ...
            event.canonical_digest = "other".into();
            ledger.record_event(target, "subscription-one", &event, 1000).unwrap();
            let mut submissions = Vec::new();
            for number in 0..2 {
                let mut identity = selection.clone();
                identity.operation_identifier = format!("local-{number}");
                let canonical = r#"{"root_path":"/content/example"}"#;
                operations.admit(&AdmissionRequest {
                    author_target_identity: "opaque-target".into(), author_target_identity_digest: target.clone(), caller_identity: None,
                    canonical_command: canonical.into(), command_fingerprint: CommandFingerprint::derive(&FingerprintInput {
            ...
            if defect.starts_with("generation-")
                && !["generation-empty", "generation-finish"].contains(&defect)
            {
                let retained = repository
                    ...
                    .unwrap();
                repository
                    .acknowledge(&retained, &["job-1".into(), "job-2".into()], 120000, 1000)
                    .unwrap();
                if defect.starts_with("generation-held-") {
                    use slingshot_domain::remote_job::{
                        AgentJobState, JobEventSequence, RemoteJobObservation,
                    };
                    let retained = repository
                        .read(target, &submissions[0].operation.agent_operation_identifier)
                        .unwrap()
                    ...
                    repository
                        .reconcile_active_snapshot(
                            &retained,
                            &["job-1".into(), "job-2".into()],
                            RemoteJobObservation {
                                state: AgentJobState::Running,
                                applied_sequence: JobEventSequence::of(
                                    if defect == "generation-held-older" { 4 } else { 3 },
                                ),
                                attempt: 1,
                                progress: 10,
                            },
            ...
            let set_recovery = || {
                ...
                let success = ["generation-settle-known-success", "generation-unanswered-success"]
                    .contains(&defect);
            ...
            if defect == "retained-contract" {
                rusqlite::Connection::open(&path)
                    .unwrap()
                    .execute_batch(
                        "UPDATE agent_operation SET command_contract_limits_digest = 'changed';",
                    )
                    .unwrap();
            }
            ...
            if defect == "generation-already-settled" {
                use slingshot_domain::operation::{
                    OperationExecutionCertainty, OperationFact, TerminalFailure,
                    TerminalFailureDisposition, TerminalFailureKind,
                };
                for member in
                    ledger.read_recovery_view(target, "subscription-one").unwrap().members()
                {
                    operations
                        .apply(
                            target,
            ...
            let peer = async {
                ...
                for stage in 0..if defect.starts_with("retained-")
                    || ["paused-exhausted", "paused-capacity"].contains(&defect)
                {
                    0
                } else if [
                    "capture",
                ...
                .contains(&defect)
                {
                    ...
                    if defect == "paused-after-capture" {
                        set_recovery();
                    }
                    if defect == "cancel" && stage == 2 {
                        cancel_ready.take().unwrap().send(()).unwrap();
                        let mut discarded = Vec::new();
                        let _ = socket.read_to_end(&mut discarded).await;
                        break;
                    }
                    let mut body = if stage == 0 {
                        serde_json::json!({
                        ...
                        let submission = &submissions
                            [if defect.starts_with("generation-") { 0 } else { stage - 1 }];
                    ...
                    let status = if defect.starts_with("generation-") {
                        if stage == 0 {
                            ...
                            || (defect == "generation-recovered" && stage == 1)
                        {
                            body = serde_json::json!({"format":"slingshot.agent/1", "transport_contract_digest":expected.transport_contract_digest,
                            ...
                            body["physical_sling_job_identifiers"] =
                                serde_json::json!(["job-1", "job-2"]);
                            if defect == "generation-held-conflict" {
                                body["progress"] = serde_json::json!(11);
                            }
                            if defect == "generation-settle-terminal" {
                                body["kind"] = serde_json::json!("succeeded");
                            }
                            if stage == 2 && defect == "generation-conflict" {
                                body["progress"] = serde_json::json!(11);
                            }
                            if stage == 2 && defect == "generation-coherent" {
                                body["progress"] = serde_json::json!(11);
                                body["sequence"] = serde_json::json!(4);
                            }
                            if stage == 2 && defect == "generation-shrunk" {
                                body["physical_sling_job_identifiers"] =
                                    serde_json::json!(["job-2"]);
                            }
                            if stage == 2 && defect == "generation-regression" {
                                body["progress"] = serde_json::json!(9);
                                body["sequence"] = serde_json::json!(4);
                            }
                            if stage == 1 && defect == "generation-reordered" {
                                body["progress"] = serde_json::json!(11);
                                body["sequence"] = serde_json::json!(4);
                            }
                            200
                        ...
                        body["kind"] = serde_json::json!("retired");
                        410
                    } else {
                        if stage == 2 && defect.starts_with("terminal") {
                            body["kind"] = serde_json::json!("succeeded");
                        }
                        if stage == 2 && defect.ends_with("expired") {
                            body["granted_retention_milliseconds"] = serde_json::json!(500);
                        }
                    ...
                    let payload = if defect == "truncated" && stage == 2 {
                        &body[..body.len() - 1]
                    } else {
                        &body
                    };
            ...
            let request = async {
                ...
                if defect == "cancel" {
                    ...
                    if defect.starts_with("generation-") && defect != "generation-already-settled" {
                        use slingshot_daemon::operation::generation_loss_probe::{
                            PhysicalRecoveryStatus,
                            probe_generation_loss_with_authentication as probe_generation_loss,
                        };
                        let SubscriptionResetOutcome::GenerationChanged(reset) =
                            result.as_ref().unwrap()
                        else {
                            panic!("generation change not captured: {defect}");
                        };
                        let mut identity = selection.clone();
                        identity.operation_identifier = first_local.clone();
                        if [
                            "generation-recovered",
                            "generation-coherent",
                            "generation-reordered",
                        ...
                        .contains(&defect)
                        {
                            use slingshot_daemon::operation::generation_recovery_dispatch::{
                                GenerationDispatchOutcome, ScheduledGenerationRecovery,
                            };
                            if mode == 4 && defect == "generation-recovered" {
                                for revision_only in [false, true] {
                                    ...
                                    for policy in [
                                        ...
                                        assert!(
                                            ScheduledGenerationRecovery::new_with_authentication(
                                                &ledger,
                                                &operations,
                                                &repository,
                                                &transport,
                                            ...
                                            .is_err(),
                                            "foreign generation recovery policy captured"
                                        );
                                        assert_eq!(
                                            operations.read(target, &first_local).unwrap(),
                                            before
                                        );
                                        assert_eq!(
                                            ledger
                            ...
                            let outcome = ScheduledGenerationRecovery::new_with_authentication(
                                &ledger,
                                &operations,
                                &repository,
                                &transport,
                        ...
                        if defect.starts_with("generation-unanswered-dispatch") {
                            ...
                            if defect.ends_with("cancel") {
                                tokio::time::pause();
                            }
                            scheduled_at.set(Some(tokio::time::Instant::now()));
                            let scheduled = ScheduledGenerationRecovery::new_with_authentication(
                                &ledger,
                                &operations,
                                &repository,
                                &transport,
                                ...
                                reset,
                                &identity,
                                if defect == "generation-unanswered-dispatch" { 900 } else { 1000 },
                            )
                            .unwrap();
                            if defect.ends_with("stale") {
                                let mut recovery = operations
                                    .read(target, &first_local)
                                    .unwrap()
                                    .unwrap()
                                    .record
                                    .outstanding_recovery
                                ...
                                operations
                                    .apply(
                                        ...
                                        &first_local,
                                        2,
                                        &slingshot_domain::operation::OperationFact::Recovery {
                                            recovery,
                                        },
                                        1000,
                                    )
                                ...
                                let GenerationDispatchOutcome::Deferred(local) =
                                    scheduled.run(None).await.unwrap()
                                else {
                                    panic!("unanswered dispatch did not defer");
                                };
                        ...
                        let report = probe_generation_loss(
                            &ledger,
                            &operations,
                            &transport,
                            &identity,
                        ...
                        assert_eq!(
                            ...
                            match defect {
                                ...
                                name if name.starts_with("generation-unanswered") =>
                                ...
                                "generation-empty" | "generation-finish" =>
                                    PhysicalRecoveryStatus::NoPhysicalJobs,
                        ...
                        if defect == "generation-finish" {
                            ...
                            identity.operation_identifier =
                                before.members()[1].identity.operation_identifier.clone();
                            let second = probe_generation_loss(
                                &ledger,
                                &operations,
                                &transport,
                                &identity,
                            ...
                            let cursor = finish_generation_reset(
                                &ledger,
                                &operations,
                                &transport,
                                &selection,
                            ...
                            if defect == "generation-unanswered-stale" {
                                set_recovery();
                            }
                            ...
                            if defect == "generation-unanswered-stale" {
                                ...
                                let local = deferred.unwrap();
                                assert!(!local.record.lifecycle_state.is_terminal());
                                ...
                                assert_eq!(
                                    u64::from(recovery.attempt_count),
                                    if defect == "generation-unanswered-cap" { cap } else { 1 }
                                );
                                assert_eq!(
                                    recovery.manual_resume_eligible,
                                    defect == "generation-unanswered-cap"
                                );
                        ...
                        } else if ["generation-missing", "generation-empty", "generation-conflict"]
                            .contains(&defect)
                        {
                            let settled = report.settle_unavailable(&transport, 1000);
                            if ["generation-missing", "generation-empty"].contains(&defect) {
                                use slingshot_domain::operation::{
                                    OperationExecutionCertainty, TerminalFailureDisposition,
                                    TerminalFailureKind,
                                };
                        ...
                        .contains(&defect)
                            || defect.starts_with("generation-settle-")
                        {
                            if defect == "generation-settle-stale" {
                                set_recovery();
                            }
                            let another = AgentJobRepository::new(
                                OperationDatabase::open(
                                    &root.path().join("physical-other.sqlite3"),
                                    settings(),
                                )
                                .unwrap(),
                            );
                            let reconciled = report
                                .reconcile_saved(
                                    if defect == "generation-settle-owner" {
                                        &another
                                    } else {
                                        &repository
                                    },
                            ...
                            .contains(&defect)
                            {
                                ...
                                assert_eq!(
                                    found.snapshot.sequence.value(),
                                    if ["generation-coherent", "generation-reordered"]
                                        .contains(&defect)
                                    {
                                        4
                                    } else {
                                ...
                                assert!(
                                    found.remaining_retention_milliseconds > 0
                                        && found.remaining_retention_milliseconds < 120000
                                );
                            }
                        } else {
                            ...
                            assert_eq!(
                                results.len(),
                                if defect == "generation-empty" { 0 } else { 2 }
                            );
            ...
            if defect.is_empty() || defect == "eligible-not-paused" {
                let SubscriptionResetOutcome::Installed { generation, cursor } =
                ...
                for child in after.members() {
                    ...
                    assert!(
                        child.remaining_retention_milliseconds > 0
                            && child.remaining_retention_milliseconds <= 120000
                    );
                }
                ...
                if defect == "generation-finish" {
                    assert!(matches!(
                        result.unwrap().unwrap(),
                        SubscriptionResetOutcome::GenerationChanged(_)
                    ));
                } else {
                    let SubscriptionResetOutcome::Installed { generation, cursor } =
                        result.unwrap().unwrap()
                ...
                for child in before.members() {
                    ...
                    assert!(
                        operations
                            .read(target, &child.identity.operation_identifier)
                            .unwrap()
                            .unwrap()
                            .record
                            .lifecycle_state
                ...
                if ["generation-recovered", "generation-coherent", "generation-reordered"]
                    .contains(&defect)
                {
                    let child = repository
                        .read(target, &submissions[0].operation.agent_operation_identifier)
                        ...
                        .unwrap();
                    assert_eq!(
                        child.observation.applied_sequence.value(),
                        if defect == "generation-recovered" { 3 } else { 4 }
                    );
                    assert_eq!(
                        child.observation.progress,
                        if defect == "generation-recovered" { 10 } else { 11 }
                    );
                    assert_eq!(child.identity.agent_event_store_generation, 7);
                ...
                assert_eq!(
                    repository
                        .physical_jobs(target, &submissions[0].operation.agent_operation_identifier)
                        .unwrap()
                        .as_slice(),
                    before
                        .physical_jobs_for(&submissions[0].operation.agent_operation_identifier)
                        .unwrap()
                );
                ...
                let SubscriptionResetOutcome::NeedsOperationRecovery { operation_identifier } =
                    result.unwrap().unwrap()
                else {
                    panic!("paused member was not deferred");
                };
            ...
            } else if defect.starts_with("missing")
                || defect.starts_with("terminal")
                || defect == "retired"
            {
                use slingshot_domain::operation::{
                    OperationExecutionCertainty, OperationFact, RecoveryCategory,
                ...
                assert_eq!(after.ledger(), before.ledger());
                assert_eq!(after.members(), before.members());
                let SubscriptionResetOutcome::CapturedOperationRecovery(handoff) =
                    result.unwrap().unwrap()
                else {
                    panic!("missing recovery handoff");
                };
                        ...
                        .apply(
                            ...
                            &OperationFact::Recovery {
                                recovery: RecoveryFact {
                                    ...
                                    retry_observed_at_unix_milliseconds: 1000,
                                },
                            },
                            1000,
                        )
                ...
                let another = AgentJobRepository::new(
                    OperationDatabase::open(&root.path().join("another.sqlite3"), settings())
                        .unwrap(),
                );
                let reconciled = if mode >= 3 {
                    handoff
                        .reconcile_saved(
                            if defect == "missing-owner" { &another } else { &repository },
                            &transport,
                            None,
                        )
                        .await
                ...
                if ["missing-stale", "missing-owner"].contains(&defect) {
                    ...
                    assert!(reconciled.is_ok(), "{defect}");
                    assert!(held.record.revision > original.record.revision);
                    if defect.ends_with("expired") {
                ...
                for child in before
                    .members()
                    .iter()
                    .filter(|child| child.identity.operation_identifier != operation)
                {
                    assert_eq!(
                        repository
                            .read(target, &child.identity.agent_operation_identifier)
                ...
                if defect == "cancel" {
                    ...
                    assert!(result.unwrap().is_err(), "http2={http2} defect={defect}");
                }
                assert_eq!(after.ledger(), before.ledger());
                assert_eq!(after.members(), before.members());
            }
            assert!(
                ...
                "unexpected request: {defect} http2={http2}"
            );
        }
    }
}
...
async fn retained_package_http2_lookup_keeps_discovery_snapshot_and_artifact_on_http2() {
    retained_artifact_completion_case(
...
async fn selected_failure_lookup_settles_only_validated_no_effect_and_keeps_unknown_open() {
    for (wire, failure) in [
        ...
        (
            ...
            r#"{"failure":"configuration_lookup_failed"}"#,
        ),
        ...
        (
            ...
            r#"{"failure":"bundle_inventory_failed"}"#,
        ),
        (
            "list_open_service_gateway_initiative_components",
            r#"{"failure":"component_inventory_failed"}"#,
        ),
        ...
        (
            ...
            r#"{"asset_path":"/content/dam/example/logo.png","failure":"asset_invalid"}"#,
        ),
        (
            "inspect_replication_queue",
            r#"{"agent_identifier":"publish","failure":"queue_inventory_failed"}"#,
        ),
        (
            "list_group_members",
            r#"{"budget":"property_values","failure":"discovery_budget_exceeded"}"#,
        ),
        (
            "inspect_sling_job",
        ...
        (
            ...
            r#"{"failure":"instance_access_denied","instance_identifier":"/var/workflow/instances/server0/2024-01-01/request-for-activation_1"}"#,
        ),
        (
            "inspect_replication_agent",
            r#"{"agent_identifier":"publish","failure":"agent_not_found"}"#,
        ),
        (
            "resolve_resource_path",
            r#"{"failure":"request_address_rejected","subject":"https://example.test/en/report.html"}"#,
        ),
        (
            "map_resource_path",
        ...
        (
            "start_workflow",
            r#"{"failure":"platform_control_outcome_unknown","model_identifier":"/var/workflow/models/request-for-activation/jcr:content/model"}"#,
        ),
        (
            "terminate_workflow_instance",
            r#"{"failure":"instance_not_found","instance_identifier":"/var/workflow/instances/server0/2024-01-01/request-for-activation_1"}"#,
        ),
        (
            "set_workflow_instance_suspension",
            r#"{"failure":"platform_control_outcome_unknown","instance_identifier":"/var/workflow/instances/server0/2024-01-01/request-for-activation_1"}"#,
        ),
        (
        ...
        (
            "create_user",
            r#"{"authorizable_identifier":"author","failure":"authorizable_already_exists"}"#,
        ),
        (
            "create_group",
            r#"{"authorizable_identifier":"author","failure":"mutation_outcome_unknown"}"#,
        ),
        (
            "delete_authorizable",
            r#"{"authorizable_identifier":"author","failure":"group_has_members"}"#,
        ),
        (
            "update_user_profile",
            r#"{"authorizable_identifier":"author","failure":"authorizable_not_found"}"#,
        ),
        (
            "set_user_disabled",
            r#"{"authorizable_identifier":"author","failure":"mutation_outcome_unknown"}"#,
        ...
        (
            "add_group_member",
            r#"{"failure":"member_not_found","group_identifier":"content-authors","member_identifier":"author"}"#,
        ),
        (
            "remove_group_member",
            r#"{"failure":"group_not_found","group_identifier":"content-authors","member_identifier":"author"}"#,
        ),
        (
            "create_content_fragment",
            r#"{"failure":"parent_not_found","target_path":"/content/dam/example/fragments/offer"}"#,
        ),
        (
            "update_content_fragment",
            r#"{"failure":"variation_not_found","fragment_path":"/content/dam/example/fragments/offer"}"#,
        ),
        (
            "delete_content_fragment",
            r#"{"failure":"fragment_is_referenced","fragment_path":"/content/dam/example/fragments/offer"}"#,
        ),
        (
            "create_experience_fragment",
            r#"{"failure":"parent_not_found","target_path":"/content/experience-fragments/example/hero"}"#,
        ),
        (
            "update_experience_fragment",
            r#"{"failure":"variation_not_found","variation_path":"/content/experience-fragments/example/hero/web"}"#,
        ),
        (
            "delete_experience_fragment",
            r#"{"failure":"mutation_outcome_unknown","fragment_path":"/content/experience-fragments/example/hero"}"#,
        ),
        (
            "create_asset",
            r#"{"failure":"parent_not_found","target_path":"/content/dam/example/logo.png"}"#,
        ),
        (
            "create_asset",
            r#"{"failure":"mutation_outcome_unknown","target_path":"/content/dam/example/logo.png"}"#,
        ),
        (
            "create_asset_folder",
        ...
        (
            "delete_asset",
            r#"{"asset_path":"/content/dam/example/logo.png","failure":"asset_is_referenced"}"#,
        ),
        (
            "update_asset_metadata",
            r#"{"asset_path":"/content/dam/example/logo.png","failure":"asset_not_found"}"#,
        ),
        ...
        (
            "move_page",
            r#"{"destination_path":"/content/archive/example","failure":"source_not_found","source_path":"/content/example"}"#,
        ),
        ("delete_page", r#"{"failure":"target_is_referenced","page_path":"/content/example"}"#),
        (
            "update_component",
            r#"{"component_path":"/content/example/jcr:content/text","failure":"property_rejected"}"#,
        ),
        (
            "delete_component",
            r#"{"component_path":"/content/example/jcr:content/text","failure":"component_not_found"}"#,
        ),
        (
            "delete_component",
            r#"{"component_path":"/content/example/jcr:content/text","failure":"mutation_outcome_unknown"}"#,
        ),
        (
            "reorder_component",
            r#"{"component_path":"/content/example/jcr:content/text","failure":"sibling_not_found"}"#,
        ),
        ("replicate_content", r#"{"failure":"source_not_found","source_path":"/content/example"}"#),
        (
            "replicate_content",
            r#"{"accepted_item_count":0,"current_path":"/content/example","failure":"admission_rejected","remaining_item_count":2}"#,
        ),
        (
            "replicate_content",
            r#"{"accepted_item_count":1,"current_path":"/content/example/child","failure":"admission_budget_exceeded","remaining_item_count":1}"#,
        ),
        (
            "replicate_content",
            r#"{"accepted_item_count":1,"current_path":"/content/example/child","failure":"admission_outcome_unknown","remaining_item_count":1}"#,
        ),
        (
            "replicate_content",
            r#"{"accepted_item_count":1,"current_path":"/other","failure":"admission_rejected","remaining_item_count":1}"#,
        ),
        ("query_paths", r#"{"failure":"root_not_found","root_path":"/content/example"}"#),
        (
            "find_pages_by_template",
            r#"{"failure":"root_access_denied","root_path":"/content/example"}"#,
        ),
        (
        ...
        (
            ...
            r#"{"failure":"root_not_found","root_path":"/content/example"}"#,
        ),
        (
        ...
        (
            ...
            r#"{"failure":"page_invalid","page_path":"/content/example"}"#,
        ),
        ...
        (
            "inspect_open_service_gateway_initiative_configuration",
            r#"{"failure":"configuration_lookup_failed","private_payload":"must-not-escape"}"#,
        ),
        ...
        (
            "create_page",
            r#"{"failure":"mutation_outcome_unknown","target_path":"/content/example/new"}"#,
        ),
        (
            "add_component",
            r#"{"failure":"parent_not_orderable","target_path":"/content/example/jcr:content/text"}"#,
        ),
        (
            "add_component",
            r#"{"failure":"mutation_outcome_unknown","target_path":"/content/example/jcr:content/text"}"#,
        ),
    ] {
        retained_artifact_completion_case(wire, false, false, false, Some(failure), false, false)
            .await;
    }
}
...
async fn retained_artifact_completion_case(
    ...
    let arguments = match wire {
        ...
        "read_content_fragment" => {
            r#"{"fragment_path":"/content/dam/example/offer","variation_name":"web"}"#
        }
        ...
        "inspect_workflow_instance" => {
            r#"{"instance_identifier":"/var/workflow/instances/server0/2024-01-01/request-for-activation_1"}"#
        }
        ...
        "update_user_profile" => {
            r#"{"authorizable_identifier":"author","removed_property_names":["givenName"]}"#
        }
        "set_user_disabled" => r#"{"authorizable_identifier":"author","disabled":true}"#,
        "add_group_member" => {
            r#"{"group_identifier":"content-authors","member_identifier":"author"}"#
        }
        "remove_group_member" => {
            r#"{"group_identifier":"content-authors","member_identifier":"author"}"#
        }
        ...
        "update_content_fragment" => {
            r#"{"elements":{"title":"Spring offer"},"fragment_path":"/content/dam/example/fragments/offer","variation_name":"web"}"#
        }
        "delete_content_fragment" => {
            r#"{"fragment_path":"/content/dam/example/fragments/offer","reference_policy":"refuse_when_referenced"}"#
        }
        "create_experience_fragment" => {
        ...
        "update_experience_fragment" => {
            r#"{"title":"Hero","variation_path":"/content/experience-fragments/example/hero/web"}"#
        }
        "delete_experience_fragment" => {
            r#"{"fragment_path":"/content/experience-fragments/example/hero","reference_policy":"refuse_when_referenced"}"#
        }
        "create_asset" => {
        ...
        "move_page" => {
            r#"{"adjust_references":true,"destination_path":"/content/archive/example","source_path":"/content/example"}"#
        }
        "delete_page" => {
            r#"{"page_path":"/content/example","reference_policy":"refuse_when_referenced"}"#
        }
        "update_component" => {
            r#"{"component_path":"/content/example/jcr:content/text","removed_property_names":["text"]}"#
        }
        "delete_component" => r#"{"component_path":"/content/example/jcr:content/text"}"#,
        "reorder_component" => {
            r#"{"component_path":"/content/example/jcr:content/text","placement":{"mode":"before","sibling_name":"image"}}"#
        }
        "create_page" => {
            r#"{"page_name":"new","parent_path":"/content/example","template_path":"/conf/example/templates/page","title":"New"}"#
        }
        "add_component" => {
            r#"{"component_name":"text","content_parent":"content_root","page_path":"/content/example","resource_type":"example/components/text"}"#
        }
        "inspect_open_service_gateway_initiative_configuration" => {
            r#"{"persistent_identifier":"example.service"}"#
        }
        "query_paths" | "find_assets_by_metadata" => r#"{"root_path":"/content/example"}"#,
        "find_pages_by_template" => {
            r#"{"root_path":"/content/example","template_path":"/conf/example/template"}"#
        }
        "find_pages_containing_phrase" => {
            r#"{"phrase":"annual report","root_path":"/content/example"}"#
        }
        "find_pages_using_components" => {
    ...
    let submission = Submission::build(
        ...
        "subscription-one",
        arguments,
        if matches!(wire, "load_content_as_json" | "download_content_package") {
            ExpectedArtifactManifest::declaring(manifest, 1, maximum).unwrap()
        } else {
            ExpectedArtifactManifest::empty()
        },
    )
    ...
    let mut operations =
        OperationRepository::new(OperationDatabase::open(&path, settings()).unwrap());
    let installation = InstallationIdentifier::parse(&"a1".repeat(DIGEST_BYTE_PAIRS)).unwrap();
    operations.admit(&AdmissionRequest {
    ...
    if let Some(failure) = failure {
        ...
        let partial = wire == "replicate_content"
            && failure.contains("\"accepted_item_count\":1")
            && !unknown
            && !invalid;
        let document = slingshot_agent_protocol::terminal_failure::TerminalFailureDocument {
            ...
            canonical_failure: failure.to_owned(),
        ...
        let protocol = slingshot_daemon::retained_author_protocol::RetainedAuthorProtocol::new_with_authentication(
            ...
            identity.clone(), submission.clone(), RECOVERY_OBSERVED_AT,
        ...
        let mut arguments: serde_json::Value =
            serde_json::from_str(&submission.canonical_arguments).unwrap();
        ...
        .unwrap();
        assert_eq!(
            matches!(answer, OperationExecutorOutcome::RecoveryRequired { .. }),
            invalid || unknown
        );
        let reopened =
        ...
        assert!(
            timeout(Duration::from_millis(NO_REQUEST_OBSERVATION_MILLISECONDS), listener.accept())
                .await
                .is_err(),
            "failure handling sent replacement work"
        );
        return;
    }
    ...
    if retire {
        ...
        for (status, reason, wrong_identity, terminal) in [
            ...
            let unavailable = serde_json::json!({
                "provenance":submission.provenance,"agent_event_store_generation":7,
                "agent_operation_identifier":submission.operation.agent_operation_identifier,
                "artifact_identifier":if wrong_identity { "4".repeat(DIGEST_HEX_CHARACTERS) } else { artifact.as_text().to_owned() },
                ...
                assert!(
                    operations
                        .begin_artifact_acquisition(
                            &retained,
                            revision,
                            &"5".repeat(DIGEST_HEX_CHARACTERS),
                            slot,
                            &digest,
                            999999
                        )
...
async fn remote_staging_reserves_first_and_never_publishes_unproved_bytes() {
    ...
    let clock = AuthenticationClock(std::cell::Cell::new(0));
    let authentication =
        slingshot_daemon::operation::author_authentication::AuthorAuthentication::Provider {
            provider: &provider,
            source: &NoTokenSource,
            clock: &clock,
        };
    ...
    let (fixed_authentication, _) =
        provider.authenticate(provider.snapshot().author().as_text(), 0, &NoTokenSource).unwrap();
    for defect in ["", "short", "digest", "long", "media", "location", "capacity"] {
        use slingshot_daemon::operation::{
            artifact_completion::stage_remote_artifact_over, subscription_reset::ResetTransport,
        };
        let isolated = tempfile::tempdir().unwrap();
        let database =
            OperationDatabase::open(&isolated.path().join("http2-stage.sqlite3"), settings())
                .unwrap();
        let capacity =
        ...
        policy.individual_artifact_bytes = 0;
        let refused = PersistentCapacityAccount::new(&database, policy);
        let store_root = isolated.path().join("artifacts");
        let store = ArtifactStore::open(&store_root).unwrap();
        let transfer = stage_remote_artifact_over(
            &transport,
            &identity,
            &submission,
            ...
            &request,
            &store,
            if defect == "capacity" { &refused } else { &capacity },
            0,
            ResetTransport::Http2,
        );
        if defect == "capacity" {
        ...
        let peer = async {
            ...
            let mut head = vec![0; length];
            socket.read_exact(&mut head).await.unwrap();
            fixed_authentication.lend_value_bytes(|value| {
                assert!(head.windows(value.len()).any(|bytes| bytes == value))
            });
            let route = format!(
            ...
            let mut block = vec![0x88];
            for (name, value) in [
                ("content-type", if defect == "media" { "text/plain" } else { "application/zip" }),
                ("content-length", "3"),
            ] {
                block.extend_from_slice(&[0, name.len() as u8]);
                block.extend_from_slice(name.as_bytes());
        ...
        let (result, ()) =
            timeout(Duration::from_secs(5), async { tokio::join!(transfer, peer) }).await.unwrap();
    ...
    let client = async {
        ...
        assert!(
            timeout(Duration::from_millis(NO_REQUEST_OBSERVATION_MILLISECONDS), &mut transfer)
                .await
                .is_err()
        );
    };
...
fn selected_snapshot_with_publisher(
    endpoint: &str,
    publisher: &str,
    ...
    for NoTokenClocks
{
    fn reading_milliseconds(&self) -> u64 {
        panic!("Basic async policy must not sample token clocks")
    }
}
    ...
    for NoTokenClocks
{
    fn sample(&self) -> Option<u64> {
        panic!("Basic async policy must not issue assertions")
    }
}
struct AuthenticationClock(std::cell::Cell<usize>);
impl slingshot_agent_connection::authentication::identity_management_exchange::MonotonicClock
    for AuthenticationClock
{
    fn reading_milliseconds(&self) -> u64 {
...
async fn selected_admission_orders_preflight_persistence_post_and_restart_recovery() {
        ...
        operations
            .admit(
                &AdmissionRequest {
                    ...
                    installation_identifier: InstallationIdentifier::parse(&"a1".repeat(DIGEST_BYTE_PAIRS))
                        .unwrap(),
                    operation_identifier: identity.operation_identifier.clone(),
                    selected_environment_revision: identity.selected_environment_revision.clone(),
                    workflow_correlation_identifier: None,
    ...
    let drift_vectors: Vec<serde_json::Value> =
        include_str!("fixtures/author-agent-conformance/provenance-drift.jsonl")
            .lines()
            .map(|line| serde_json::from_str(line).unwrap())
            .collect();
    let async_selected_provider = async_provider(&endpoint);
    for asynchronous in [false, true] {
        for field in drift_vectors
            .iter()
            .map(|vector| vector["field"].as_str().unwrap())
            .chain(["agent_event_store_generation", "continuation_authority_ready"])
        {
            ...
            match field {
                ...
                "command_wire_name" => {
                    incompatible["command_contracts"][0][field] = serde_json::json!("create_page")
                }
                "command_semantic_contract_version" => {
                    incompatible["command_contracts"][0][field] = serde_json::json!("second")
                }
                "argument_schema_digest"
                | "result_schema_digest"
                | "command_contract_limits_digest" => {
                    incompatible["command_contracts"][0][field] =
                        serde_json::json!("0".repeat(DIGEST_HEX_CHARACTERS))
                }
    ...
    for case in 0..16 {
        ...
        let identity = ExecutionIdentity {
            operation_identifier: if asynchronous {
                match scenario {
                    10 => "async-accepted",
                    11 => "async-lost-ack",
                    _ => "async-stale-before-post",
                }
            } else if provider_owned {
                match scenario {
                    10 => "provider-accepted",
                    11 => "provider-lost-ack",
                    _ => "provider-stale-before-post",
                }
            } else if scenario == 9 {
                "automatic-accepted"
            } else if http2 {
        ...
        if scenario == 0 {
            for revision_only in [false, true] {
                let foreign_endpoint =
                    if revision_only { endpoint.clone() } else { format!("{endpoint}/foreign") };
                let foreign_publisher = if revision_only {
                    "http://another-publisher.example.com"
                } else {
                    "http://publish.example.com"
                };
                let foreign = EnvironmentAuthenticationProvider::new(
                    selected_snapshot_with_publisher(&foreign_endpoint, foreign_publisher),
                    2,
                );
                ...
                assert_ne!(foreign.snapshot().revision(), provider.snapshot().revision());
                let (foreign_fixed, _) =
                    foreign.authenticate(&foreign_endpoint, 0, &NoTokenSource).unwrap();
                let before = repository
                    .read(
                        &identity.author_target_identity_digest,
                        &submission.operation.agent_operation_identifier,
        ...
        let authentication_policy = if asynchronous {
            ...
            AuthorAuthentication::Fixed {
                authentication: &authentication,
                protocol: if scenario == 9 {
                    slingshot_daemon::operation::subscription_reset::ResetTransport::Automatic
                } else if http2 {
                    slingshot_daemon::operation::subscription_reset::ResetTransport::Http2
                } else {
                    slingshot_daemon::operation::subscription_reset::ResetTransport::Http1
                },
            }
        };
        let protocol = slingshot_daemon::retained_author_protocol::RetainedAuthorProtocol::new_with_authentication(
        ...
        let peer = async {
            for stage in 0..if stale_before_post { 3 } else { 4 } {
                ...
                if http2 {
                    ...
                    socket.read_exact(&mut frame).await.unwrap();
                    assert_eq!((frame[3], frame[4]), (1, if stage == 3 { 4 } else { 5 }));
                    let length = usize::from(frame[0]) << 16
                    ...
                    let retained = observer
                        ...
                        .unwrap();
                    if stage == 0 {
                        assert!(retained.is_none(), "HTTP/2 admission preceded capability");
                    } else {
                        assert_eq!(
                    ...
                    if stage == 2 && stale_before_post {
                        concurrent
                            .apply(
                                ...
                                1,
                                &slingshot_domain::operation::OperationFact::Progress {
                                    detail: "concurrent local change".into(),
                                },
                                1000,
                            )
                            .unwrap();
                    ...
                    let body = match stage {
                        0 | 1 => capability.as_str(),
                        2 => r#"{"token":"one-use-token"}"#,
                        _ => acknowledgement.as_str(),
                    };
                    let mut block =
                        if stage == 3 { vec![8, 3, b'2', b'0', b'2'] } else { vec![0x88] };
                    for (name, value) in [
                        ("content-type", "application/json".to_owned()),
                        ("content-length", body.len().to_string()),
                    ] {
        ...
        .await
        .unwrap();
        if provider_owned && !asynchronous {
            assert_eq!(
                clock.0.get(),
                3,
                "capability-before/after admission and submission sample fresh clock readings"
            );
        }
        if stale_before_post {
            assert_eq!(
        ...
        if provider_owned {
            ...
            let repeated = slingshot_daemon::operation::durable_author_submission::submit_initial_with_authentication(
                ...
                authentication_policy,RECOVERY_OBSERVED_AT,
            ).await.unwrap();
            assert!(matches!(
                repeated,
                SubmissionOutcome::SubmissionUnknown {
                    cause:
                        slingshot_agent_connection::command_submission::UnknownCause::LookupRequired,
                }
            ));
        ...
        assert_eq!(
            clock.0.get(),
            before_resume_clock,
            "retained product restart must not acquire credentials"
        );
        assert_eq!(
            handoff,
        ...
        let revision = if truncate {
            ...
            for (local_revision, concurrent_event) in [(2, true), (2, false), (4, false)] {
                ...
                assert_eq!(
                    ...
                    if local_revision == 2 { 4 } else { 5 },
                    "missing result did not consume acquisition attempt"
                );
                assert!(!held.record.lifecycle_state.is_terminal());
        ...
        if truncate {
            ...
            slingshot_storage::maintenance::apply(reopened.database(), &reviewed, MAINTENANCE_AT)
                .unwrap();
            assert!(
                reopened
                    .read(
