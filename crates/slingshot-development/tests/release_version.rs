...
fn the_declared_release_tag_is_accepted() {
    ...
    assert!(String::from_utf8_lossy(&output.stdout).contains("both name 0.0.0"));
}
...
fn the_release_prefix_is_part_of_the_contract() {
    let output = run(Some("tag"), Some("0.0.0"));
    assert!(!output.status.success());
...
fn a_manual_run_reports_the_declared_version_without_inventing_a_tag() {
    ...
    assert!(output.status.success());
    assert!(String::from_utf8_lossy(&output.stdout).contains("publishes 0.0.0"));
}
