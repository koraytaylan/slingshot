...
#[path = "agent_subscription_ledger_events.rs"]
mod events;
...
impl AgentSubscriptionLedger {
    ...
    pub fn record_completed_event_cursor(
        ...
        events::require_completed_identity(&self.database, expected, fact, physical)?;
        if now < expected.member.recorded_at_unix_milliseconds
            || now > i64::MAX as u64
            || fact.event_bytes > i64::MAX as u64
        {
            return Err(AgentRepositoryFailure::Conflicted);
        }
        events::require_event_digest(&fact.canonical_digest)?;
    ...
    ///
    /// # Errors
    /// Refuses absent subscriptions, inconsistent retained membership, capacity
    /// violations and database failures while reading or committing the snapshot.
    ...
    ///
    /// # Errors
    /// Refuses stale membership, invalid boundaries or snapshots, exhausted
    /// capacity and database failures. Any failure rolls back the whole reset.
    ...
    ///
    /// # Errors
    /// Refuses foreign or stale views, invalid boundaries, nonempty membership
    /// and database failures. No partial boundary update is committed.
    ...
    /// Returns [`AgentRepositoryFailure::NoSuchSubscription`].
    ///
    /// Compaction also returns [`AgentRepositoryFailure::Conflicted`] for an
    /// invalid floor, a floor beyond the retained cursor, or a floor below an
    /// already compacted boundary. Refusals leave retained evidence unchanged.
    pub fn compact_below(
        ...
        require_cursor_bound(floor_cursor)?;
        let connection = self.database.connection();
        let transaction = write_transaction(connection)?;
        ...
        if held.cursor.as_deref().is_none_or(|cursor| {
            slingshot_domain::stream_cursor_order::compare(floor_cursor, cursor).is_gt()
        }) || held.compacted_below_cursor.as_deref().is_some_and(|floor| {
            slingshot_domain::stream_cursor_order::compare(floor_cursor, floor).is_lt()
        }) {
            return Err(AgentRepositoryFailure::Conflicted);
        }
        Self::require_reconciled(
...
fn require_boundary(
    ...
    if !core::ptr::eq(expected.owner, owner)
        ...
        || (generation == expected.ledger.agent_event_store_generation
            && expected.ledger.cursor.as_deref().is_some_and(|held| {
                slingshot_domain::stream_cursor_order::compare(cursor, held).is_lt()
            }))
    {
        return Err(AgentRepositoryFailure::Conflicted);
    }
...
fn classify(held: &SubscriptionLedgerRow, fact: &EventFact) -> LedgerOutcome {
    ...
    if slingshot_domain::stream_cursor_order::compare(&fact.cursor, cursor).is_gt() {
        return LedgerOutcome::Advanced;
    }
    if slingshot_domain::stream_cursor_order::compare(&fact.cursor, cursor).is_lt() {
...
fn require_active_snapshot(
    member: &AgentSubmission,
    snapshot: &ActiveResetSnapshot,
    physical_jobs: &[String],
    generation: u64,
    cursor: &str,
    now_unix_milliseconds: u64,
) -> Result<(), AgentRepositoryFailure> {
    ...
    if member.identity.agent_event_store_generation != generation
        ...
        || slingshot_domain::stream_cursor_order::compare(&snapshot.subscription_watermark, cursor)
            .is_lt()
        ...
        || physical_jobs.iter().any(|job| !snapshot.physical_sling_job_identifiers.contains(job))
    {
        return Err(AgentRepositoryFailure::Conflicted);
    }
    Ok(())
