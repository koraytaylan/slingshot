//! Moving an asset, and deciding what happens to what pointed at it.
//!
//! The same two decisions a page move carries, for the same reasons: every page
//! that rendered the asset refers to its old address, and a destination inside
//! the source is a request nothing can carry out. The containment rule is the
//! shared one rather than a second copy, so the two commands cannot come to
...
/// One request to move an asset.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(deny_unknown_fields)]
pub struct MoveAssetCommand {
    /// Whether references to the old address follow the asset.
    pub adjust_references: bool,
    /// Address the asset is moved to.
    pub destination_path: RepositoryPath,
    /// Address the asset is moved from.
    pub source_path: RepositoryPath,
}

impl MoveAssetCommand {
...
impl MoveAssetRefusal {
    ...
    pub fn require_answers(&self, command: &MoveAssetCommand) -> Result<(), MutationResultFailure> {
        let adjusting = matches!(self.failure, MoveAssetFailure::ReferenceAdjustmentBudgetExceeded);
...
/// What a completed asset move did.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(transparent)]
pub struct MoveAssetResult {
    /// Where the asset went, and what the move rewrote on the way.
    pub moved: MovedResourceResult,
}

impl MoveAssetResult {
