...
fn targeted_read_failures_bind_subjects_and_selected_categories() {
    ...
    for (arguments, field, subject) in [
        ...
        (serde_json::json!({"command":"read_content_fragment","fragment_path":"/content/dam/example/offer","variation_name":"web"}), "fragment_path", "/content/dam/example/offer"),
        (serde_json::json!({"command":"list_child_pages","root_path":"/content/example"}), "root_path", "/content/example"),
        (serde_json::json!({"command":"list_group_members","group_identifier":"authors","include_indirect":false}), "group_identifier", "authors"),
        (serde_json::json!({"command":"list_asset_renditions","asset_path":"/content/dam/example/logo.png"}), "asset_path", "/content/dam/example/logo.png"),
        (serde_json::json!({"command":"inspect_replication_queue","agent_identifier":"publish"}), "agent_identifier", "publish"),
        (serde_json::json!({"command":"inspect_sling_job","job_identifier":"2024/01/01/example-job-1"}), "job_identifier", "2024/01/01/example-job-1"),
        (serde_json::json!({"command":"inspect_workflow_instance","instance_identifier":"/var/workflow/instances/server0/2024-01-01/request-for-activation_1"}), "instance_identifier", "/var/workflow/instances/server0/2024-01-01/request-for-activation_1"),
        (serde_json::json!({"command":"inspect_replication_agent","agent_identifier":"publish"}), "agent_identifier", "publish"),
        (serde_json::json!({"command":"resolve_resource_path","include_trace":false,"request_address":"https://example.test/en/report.html"}), "subject", "https://example.test/en/report.html"),
        (serde_json::json!({"command":"map_resource_path","include_trace":false,"repository_path":"/content/example"}), "subject", "/content/example"),
    ] {
        let initial: Command = serde_json::from_value(arguments.clone()).unwrap();
        let windowed = matches!(initial, Command::ListChildPages(_) | Command::ListGroupMembers(_) | Command::ListAssetRenditions(_) | Command::InspectReplicationQueue(_)
        ...
        if windowed {
            arguments["result_window"] = serde_json::to_value(slingshot_domain::command::result_window::ResultWindow::continuation("opaque-token").unwrap()).unwrap();
        }
        let command: Command = serde_json::from_value(arguments).unwrap();
        let expected = ResultExpectation {
            operation: WireOperationIdentity::of(&"a".repeat(64), &"b".repeat(64), "local-one", AgentEventStoreGeneration::of(7)),
            daemon_subscription_identifier: "subscription-one".to_owned(),
            expected_provenance: ExpectedProvenance {
                command_contract: SelectedCommandContractIdentity::installed(command.wire_name()).unwrap(),
            ...
            }, submitted_command_digest: "c".repeat(64), wire_name: command.wire_name().to_owned(),
        };
        ...
        for category in &CommandCatalog::published().find(command.wire_name()).unwrap().failure_categories {
            let value = if category == "discovery_budget_exceeded" { serde_json::json!({"budget":"candidate_nodes","failure":category}) }
                else if category == "configuration_lookup_budget_exceeded" { serde_json::json!({"budget":"lookup_duration","failure":category}) }
                else if category.starts_with("continuation_token_") { serde_json::json!({"failure":category}) }
                else if field.is_empty() { serde_json::json!({"failure":category}) }
                else { serde_json::json!({"failure":category,field:subject}) };
            let decoded = decode(&value).unwrap();
            assert_eq!(decoded.category(), category);
            assert_eq!(format!("{decoded:?}"), "ValidatedReadFailure([redacted])");
            for (member, changed) in [(field, serde_json::json!(format!("{subject}-other"))),
                ("failure", serde_json::json!("unknown_category")), ("private_payload", serde_json::json!("private")),
                ("matches", serde_json::json!([])), ("next_continuation_token", serde_json::json!("private"))] {
                let mut invalid = value.clone(); invalid[member] = changed;
            ...
            if category.starts_with("continuation_token_") { assert!(decode_with(&value, &initial).is_err()); }
            if category == "configuration_lookup_budget_exceeded" {
                assert!(decode(&serde_json::json!({"budget":"matching_configurations","failure":category})).is_ok());
                assert!(decode(&serde_json::json!({"budget":"property_values","failure":category})).is_err());
            }
            if category == "variation_not_found" {
                let mut master = command.clone();
                if let Command::ReadContentFragment(asked) = &mut master { asked.variation_name = None; }
                assert!(decode_with(&value, &master).is_err());
            }
        }
        if windowed {
            for budget in ["candidate_nodes", "property_values", "property_bytes", "criterion_evaluations", "execution_duration"] {
                assert!(decode(&serde_json::json!({"budget":budget,"failure":"discovery_budget_exceeded"})).is_ok());
            }
        ...
        if matches!(command, Command::MapResourcePath(_)) {
            assert!(decode(&serde_json::json!({"failure":"request_address_rejected","subject":subject})).is_err());
        }
        if field.is_empty() {
            ...
            let foreign_category = if matches!(command, Command::FindSlingJobs(_) | Command::ListSlingJobQueues(_)) {
...
fn authoring_mutation_failures_use_typed_request_policy() {
    ...
    check_mutation_failure_cases(&[
        (serde_json::json!({"command":"create_content_fragment","model_path":"/conf/example/settings/dam/cfm/models/offer","name":"offer","parent_path":"/content/dam/example/fragments"}), include_str!("../../slingshot-domain/tests/fixtures/commands/create_content_fragment/failures.jsonl")),
        (serde_json::json!({"command":"update_content_fragment","elements":{"title":"Spring offer"},"fragment_path":"/content/dam/example/fragments/offer","variation_name":"web"}), include_str!("../../slingshot-domain/tests/fixtures/commands/update_content_fragment/failures.jsonl")),
        (serde_json::json!({"command":"delete_content_fragment","fragment_path":"/content/dam/example/fragments/offer","reference_policy":"refuse_when_referenced"}), include_str!("../../slingshot-domain/tests/fixtures/commands/delete_content_fragment/failures.jsonl")),
        (serde_json::json!({"command":"create_experience_fragment","name":"hero","parent_path":"/content/experience-fragments/example","template_path":"/conf/example/settings/wcm/templates/experience-fragment","variation_name":"web"}), include_str!("../../slingshot-domain/tests/fixtures/commands/create_experience_fragment/failures.jsonl")),
        (serde_json::json!({"command":"update_experience_fragment","title":"Hero","variation_path":"/content/experience-fragments/example/hero/web"}), include_str!("../../slingshot-domain/tests/fixtures/commands/update_experience_fragment/failures.jsonl")),
        (serde_json::json!({"command":"delete_experience_fragment","fragment_path":"/content/experience-fragments/example/hero","reference_policy":"refuse_when_referenced"}), include_str!("../../slingshot-domain/tests/fixtures/commands/delete_experience_fragment/failures.jsonl")),
        (serde_json::json!({"command":"create_asset","name":"logo.png","parent_path":"/content/dam/example","payload":{"media_type":"image/png","encoded_content":"aGVsbG8="}}), include_str!("../../slingshot-domain/tests/fixtures/commands/create_asset/failures.jsonl")),
        (serde_json::json!({"command":"create_asset_folder","name":"example","parent_path":"/content/dam"}), include_str!("../../slingshot-domain/tests/fixtures/commands/create_asset_folder/failures.jsonl")),
        (serde_json::json!({"command":"move_asset","source_path":"/content/dam/example/logo.png","destination_path":"/content/dam/archive/logo.png","adjust_references":true}), include_str!("../../slingshot-domain/tests/fixtures/commands/move_asset/failures.jsonl")),
        (serde_json::json!({"command":"delete_asset","asset_path":"/content/dam/example/logo.png","reference_policy":"refuse_when_referenced"}), include_str!("../../slingshot-domain/tests/fixtures/commands/delete_asset/failures.jsonl")),
        (serde_json::json!({"command":"update_asset_metadata","asset_path":"/content/dam/example/logo.png","removed_property_names":["dc:title"]}), include_str!("../../slingshot-domain/tests/fixtures/commands/update_asset_metadata/failures.jsonl")),
        (serde_json::json!({"command":"update_page","page_path":page,"title":"Annual Report"}), include_str!("../../slingshot-domain/tests/fixtures/commands/update_page/failures.jsonl")),
        (serde_json::json!({"command":"move_page","source_path":page,"destination_path":"/content/archive/report","adjust_references":true}), include_str!("../../slingshot-domain/tests/fixtures/commands/move_page/failures.jsonl")),
        (serde_json::json!({"command":"delete_page","page_path":page,"reference_policy":"refuse_when_referenced"}), include_str!("../../slingshot-domain/tests/fixtures/commands/delete_page/failures.jsonl")),
        (serde_json::json!({"command":"update_component","component_path":component,"removed_property_names":["text"]}), include_str!("../../slingshot-domain/tests/fixtures/commands/update_component/failures.jsonl")),
        (serde_json::json!({"command":"delete_component","component_path":component}), include_str!("../../slingshot-domain/tests/fixtures/commands/delete_component/failures.jsonl")),
        (serde_json::json!({"command":"reorder_component","component_path":component,"placement":{"mode":"before","sibling_name":"image"}}), include_str!("../../slingshot-domain/tests/fixtures/commands/reorder_component/failures.jsonl")),
    ]);
}
...
fn identity_mutation_failures_use_typed_request_policy() {
    check_mutation_failure_cases(&[
        (serde_json::json!({"command":"create_user","authorizable_identifier":"author"}), include_str!("../../slingshot-domain/tests/fixtures/commands/create_authorizable/failures.jsonl")),
        (serde_json::json!({"command":"create_group","authorizable_identifier":"author"}), include_str!("../../slingshot-domain/tests/fixtures/commands/create_authorizable/failures.jsonl")),
        (serde_json::json!({"command":"delete_authorizable","authorizable_identifier":"author","expected_kind":"group"}), include_str!("../../slingshot-domain/tests/fixtures/commands/delete_authorizable/failures.jsonl")),
        (serde_json::json!({"command":"update_user_profile","authorizable_identifier":"author","removed_property_names":["givenName"]}), include_str!("../../slingshot-domain/tests/fixtures/commands/update_user_profile/failures.jsonl")),
        (serde_json::json!({"command":"set_user_disabled","authorizable_identifier":"author","disabled":true}), include_str!("../../slingshot-domain/tests/fixtures/commands/set_user_disabled/failures.jsonl")),
        (serde_json::json!({"command":"add_group_member","group_identifier":"content-authors","member_identifier":"author"}), include_str!("../../slingshot-domain/tests/fixtures/commands/group_membership/failures.jsonl")),
        (serde_json::json!({"command":"remove_group_member","group_identifier":"content-authors","member_identifier":"author"}), include_str!("../../slingshot-domain/tests/fixtures/commands/group_membership/failures.jsonl")),
    ]);
}
...
fn operational_mutation_failures_use_typed_request_policy() {
    check_mutation_failure_cases(&[
        (serde_json::json!({"command":"update_open_service_gateway_initiative_configuration","assignments":{"host":{"cardinality":"scalar","type":"string","value":"example.test"}},"persistent_identifier":"com.example.service.Configuration"}), include_str!("../../slingshot-domain/tests/fixtures/commands/update_open_service_gateway_initiative_configuration/failures.jsonl")),
        (serde_json::json!({"command":"delete_open_service_gateway_initiative_configuration","persistent_identifier":"com.example.service.Configuration"}), include_str!("../../slingshot-domain/tests/fixtures/commands/delete_open_service_gateway_initiative_configuration/failures.jsonl")),
        (serde_json::json!({"command":"set_open_service_gateway_initiative_bundle_state","symbolic_name":"com.example.bundle","transition":"start"}), include_str!("../../slingshot-domain/tests/fixtures/commands/set_open_service_gateway_initiative_bundle_state/failures.jsonl")),
        (serde_json::json!({"command":"cancel_sling_job","job_identifier":"2024/01/01/example-job-1"}), include_str!("../../slingshot-domain/tests/fixtures/commands/cancel_sling_job/failures.jsonl")),
        (serde_json::json!({"command":"start_workflow","model_identifier":"/var/workflow/models/request-for-activation/jcr:content/model","payload_path":"/content/example/en/report"}), include_str!("../../slingshot-domain/tests/fixtures/commands/start_workflow/failures.jsonl")),
        (serde_json::json!({"command":"terminate_workflow_instance","instance_identifier":"/var/workflow/instances/server0/2024-01-01/request-for-activation_1"}), include_str!("../../slingshot-domain/tests/fixtures/commands/terminate_workflow_instance/failures.jsonl")),
        (serde_json::json!({"command":"set_workflow_instance_suspension","instance_identifier":"/var/workflow/instances/server0/2024-01-01/request-for-activation_1","requested_state":"suspended"}), include_str!("../../slingshot-domain/tests/fixtures/commands/set_workflow_instance_suspension/failures.jsonl")),
        (serde_json::json!({"command":"flush_replication_queue","agent_identifier":"publish","expected_entry_count":1}), include_str!("../../slingshot-domain/tests/fixtures/commands/flush_replication_queue/failures.jsonl")),
        (serde_json::json!({"command":"retry_replication_queue_entry","agent_identifier":"publish","entry_identifier":"queue-entry-1"}), include_str!("../../slingshot-domain/tests/fixtures/commands/retry_replication_queue_entry/failures.jsonl")),
    ]);
}
...
fn check_mutation_failure_cases(cases: &[(serde_json::Value, &str)]) {
    ...
    for (arguments, fixtures) in cases {
        ...
        for line in fixtures.lines() {
            ...
            let decoded = decode(&value, &command).unwrap_or_else(|_| panic!("{}: {line}", command.wire_name()));
            ...
            for field in ["persistent_identifier", "symbolic_name", "job_identifier", "model_identifier", "instance_identifier", "agent_identifier", "entry_identifier", "authorizable_identifier", "group_identifier", "member_identifier", "fragment_path", "variation_path", "asset_path", "target_path", "page_path", "component_path", "source_path", "destination_path", "failure", "private_payload"] {
                let mut changed = value.clone(); changed[field] = if field.ends_with("_identifier") || field == "symbolic_name" { "another-identity" } else { "/unrelated" }.into();
                assert!(decode(&changed, &command).is_err(), "{field}: {line}");
            }
            let mut changed = arguments.clone();
            match decoded.category() {
                "queue_expectation_mismatch" => { changed.as_object_mut().unwrap().remove("expected_entry_count"); },
                "group_has_members" => changed["expected_kind"] = "user".into(),
                "target_is_referenced" | "asset_is_referenced" | "fragment_is_referenced" => changed["reference_policy"] = "ignore_references".into(),
                "variation_not_found" if command.wire_name() == "update_content_fragment" => { changed.as_object_mut().unwrap().remove("variation_name"); },
        ...
        let declared: std::collections::BTreeSet<_> = catalog.find(command.wire_name()).unwrap().failure_categories.iter().cloned().collect();
        assert_eq!(covered, declared, "mutation failure fixtures must cover the installed registry exactly");
    }
}

#[test]
fn replication_failures_keep_zero_partial_and_unknown_admission_distinct() {
    use slingshot_agent_connection::terminal_failure::{decode_replication_failure, ReplicationFailureEffect};
    use slingshot_domain::command::catalog::Command;
    ...
    for category in ["source_not_found", "source_access_denied", "candidate_limit_exceeded", "traversal_budget_exceeded"] {
        cases.push((serde_json::json!({"failure":category,"source_path":"/content/example"}), ReplicationFailureEffect::NoAdmission));
    }
    ...
    for (value, effect) in cases {
        ...
        for (field, invalid) in [("source_path", serde_json::json!("/other")), ("current_path", serde_json::json!("/other")),
            ("accepted_item_count", serde_json::json!(u64::MAX)), ("remaining_item_count", serde_json::json!(0)),
            ("failure", serde_json::json!("unknown_category")), ("private_payload", serde_json::json!("private"))] {
            let mut changed = value.clone(); changed[field] = invalid;
...
fn discovery_failures_correlate_all_six_commands_without_partial_pages() {
    ...
    for arguments in [
        ...
        for (category, field, belongs) in [
            ("root_not_found", "root_path", !page), ("root_access_denied", "root_path", !page),
            ("page_not_found", "page_path", page), ("page_access_denied", "page_path", page), ("page_invalid", "page_path", page),
        ] {
            let value = serde_json::json!({"failure":category,field:"/content/example"});
            assert_eq!(decode(&value, &command).is_ok(), belongs);
            if belongs { valid.push(value.clone()); }
            for path in ["/other", "/content/example/child", "/content"] {
                let mut changed = value.clone(); changed[field] = path.into();
        ...
        assert!(decode(&serde_json::json!({"failure":"discovery_budget_exceeded","budget":"result_bytes"}), &command).is_err());
        let mut continued = arguments;
        continued["result_window"] = serde_json::to_value(ResultWindow::continuation("opaque-token").unwrap()).unwrap();
        ...
        for category in slingshot_domain::command::result_window::CONTINUATION_FAILURE_PRECEDENCE {
            ...
            let mut partial = value; partial["next_continuation_token"] = "private".into();
        ...
        for value in valid {
            ...
            for field in ["matches", "next_continuation_token", "count", "private_payload"] {
                let mut partial = value.clone(); partial[field] = "private".into();
...
fn content_failures_are_closed_correlated_and_keep_publication_uncertainty() {
    use slingshot_agent_connection::terminal_failure::{decode_load_failure, decode_package_failure};
    use slingshot_domain::command::catalog::Command;
    for (arguments, fixtures, package) in [
        (serde_json::json!({"command":"load_content_as_json","path":"/content/example","depth":1}),
         include_str!("../../slingshot-domain/tests/fixtures/commands/load_content_as_json/failures.jsonl"), false),
        (serde_json::json!({"command":"download_content_package","package_name":"example","roots":["/content/example"],"inclusion_filters":["/content/example/a/(.*)","/content/example/b/(.*)","/content/example/c/(.*)"],"exclusion_filters":["/content/example/a/private/(.*)","/content/example/b/private/(.*)","/content/example/c/private/(.*)"]}),
         include_str!("../../slingshot-domain/tests/fixtures/commands/download_content_package/failures.jsonl"), true),
    ] {
        let command: Command = serde_json::from_value(arguments).unwrap();
        let expected = ResultExpectation {
            operation: WireOperationIdentity::of(&"a".repeat(64), &"b".repeat(64), "local-one", AgentEventStoreGeneration::of(7)),
        ...
        for line in fixtures.lines() {
            ...
            let original: serde_json::Value = serde_json::from_str(&document.canonical_failure).unwrap();
            // Domain fixtures pin closed shapes, not transport canonical ordering.
            document.canonical_failure = slingshot_domain::command::canonical_json::write_canonical(&original).unwrap();
            ...
            if package {
                ...
                assert_eq!(decoded.proves_no_publication(), fixture["proves_no_publication"].as_bool().unwrap());
            ...
            let rejected = |document: &TerminalFailureDocument, expected: &ResultExpectation| {
                let bytes = serde_json::to_vec(document).unwrap();
                if package { decode_package_failure(&bytes, expected, &command).is_err() }
                else { decode_load_failure(&bytes, expected, &command).is_err() }
            };
