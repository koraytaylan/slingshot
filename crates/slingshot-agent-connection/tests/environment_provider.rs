...
fn provider_from_loaded_with_platform(
    ...
    EnvironmentAuthenticationProvider::new(
        snapshot_from_loaded_with_platform(loaded, profile, environment, platform),
        CACHE_IDENTITY,
    )
}
        ...
        for defect in [
            ...
            let peer = async {
                ...
                if http2 {
                    ...
                    socket
                        .write_all(&[length[1], length[2], length[3], 0, 1, 0, 0, 0, 1])
                        .await
                        .unwrap();
            ...
            let (result, ()) =
                timeout(Duration::from_secs(5), async { tokio::join!(request, peer) })
                    .await
                    .unwrap();
            ...
            [(409, ""), (410, ""), (409, "cursor"), (410, "bare"), (409, "truncated")]
        {
            ...
            let peer = async {
                ...
                if http2 {
                    ...
                    let payload =
                        if defect == "truncated" { &body[..body.len() - 1] } else { &body };
    ...
    for mode in 0..7 {
            ...
            [("", 7, false), ("sub", 0, false), ("sub", 7, true)]
        {
            ...
            let result = if mode == 6 {
                ...
                transport
                    .capture_high_water_http2(&selected, subscription, generation, &authentication)
                    .await
            } else {
                transport
                    .capture_high_water_http1(&selected, subscription, generation, &authentication)
                    .await
            };
        ...
        for (status, defect) in [
            ...
            let peer = async {
                ...
                let read_request = |mut stream: Box<dyn Peer>, body_expected: bool| async move {
                    let mut request = Vec::new();
                    if http2 {
                        let mut preface = [0; 39];
                        stream.read_exact(&mut preface).await.unwrap();
                    ...
                    assert!(
                        !request
                            .windows(b"last-event-id".len())
                            .any(|bytes| bytes == b"last-event-id")
                    );
                    if body_expected {
                        assert!(
                            request
                ...
                let serve_json = |mut stream: Box<dyn Peer>,
                    ...
                    let payload = if defect == "truncated" && allow_truncation {
                        &body[..body.len() - 1]
                    } else {
                        body
                    };
                ...
                let (stream, _) = read_request(stream, false).await;
                serve_json(stream, 200, b"{\"token\":\"test-csrf\"}".to_vec(), false).await;
                let stream = accept_stream().await;
                let (stream, request) = read_request(stream, true).await;
                if http2 {
            ...
            let request = async {
                if mode == 6 {
                    ...
                    transport
                        .capture_high_water_authenticated(
                            &identity, "sub /?", 7, &provider, &source, READING,
                        )
                        .await
                } else if automatic {
                    transport
                        .capture_high_water_negotiated(&identity, "sub /?", 7, &authentication)
                        .await
                } else if http2 {
                    transport
                        .capture_high_water_http2(&identity, "sub /?", 7, &authentication)
                        .await
                } else {
                    transport
                        .capture_high_water_http1(&identity, "sub /?", 7, &authentication)
                        .await
                }
            };
            let (result, ()) =
...
async fn selected_tls_protocol_negotiation_never_silently_downgrades_http2() {
    ...
    for version in [&rustls::version::TLS12, &rustls::version::TLS13] {
        for (mode, protocols, expected, accepted) in [
            ...
            let ((), protocol) =
                timeout(Duration::from_secs(10), async { tokio::join!(client, peer) })
                    .await
                    .unwrap();
            if accepted {
        ...
        for scenario in [
            ...
            let peer = async {
                ...
                for attempt in 0..=last {
                    ...
                    let status = if event {
                        if scenario == "event-short" || (scenario == "event" && attempt == 1) {
                            200
                        } else {
                            401
                        }
                    ...
                    } else if post && attempt == 0 {
                        200
                    } else if scenario == "post-403" || scenario == "forbidden" {
                        403
                    ...
                    let body = if event && status == 200 {
                        if scenario == "event-short" {
                            ": alive\n\nid: unfinished".into()
                        } else {
                            ": alive\n\n".into()
                        }
                    } else if scenario == "physical-lookup" && status == 404 {
                        serde_json::json!({
            ...
            if event {
                if scenario.ends_with("short") || (asynchronous && cloud) {
                    ...
                    assert_eq!(
                        result.unwrap().0,
                        if cloud && scenario == "event" { 200 } else { 401 }
                    );
                }
                assert_eq!(
                    heartbeats,
                ...
                assert_eq!(result.unwrap().0, if cloud { 200 } else { 401 });
            } else if artifact {
                ...
                if retries {
                    assert!(receipt.1 >= 20, "retry discarded first exchange time");
                }
            }
            assert_eq!(
...
async fn finite_http_refuses_ambiguous_truncated_and_surplus_wire_bytes() {
    ...
    for automatic in [false, true] {
            ...
            rejected.iter().map(|r| (*r, false)).chain(accepted.iter().map(|r| (*r, true)))
        {
            ...
            let (result, ()) =
                timeout(Duration::from_secs(5), async { tokio::join!(exchange, peer) })
                    .await
...
async fn selected_submission_sends_bound_bytes_once_and_validates_the_answer() {
    ...
    for (media, wrong_echo, accepted) in [
        ...
        let job_sets: Vec<serde_json::Value> = if media == "application/json" && !wrong_echo {
            ...
            vec![
                serde_json::json!({"name":"baseline", "identifiers":["job-one"], "acceptable":true}),
            ]
        };
        ...
        for automatic in [false, true] {
            ...
            for defect in ["", "token", "guard", "truncated"] {
                let peer = async {
                    for stage in (if automatic { -1 } else { 0 })
                        ..(if automatic && defect.is_empty() { 3 } else { 2 })
                    {
                        if stage == 1 && ["token", "guard"].contains(&defect) {
                            break;
                        }
                        let (socket, _) = listener.accept().await.unwrap();
                        trait Peer: tokio::io::AsyncRead + tokio::io::AsyncWrite + Unpin {}
                        ...
                        let mut socket: Box<dyn Peer> = if automatic {
                            let mut configuration = rustls::ServerConfig::builder_with_provider(
                        ...
                        if stage != 1 {
                            assert!(!head.windows(10).any(|part| part == b"csrf-token"));
                        } else {
                            for value in [
                                "csrf-test-value",
                                ...
                                assert!(
                                    head.windows(value.len()).any(|part| part == value.as_bytes())
                                );
                            }
                        ...
                        let mut block =
                            if stage != 1 { vec![0x88] } else { vec![0x08, 3, b'2', b'0', b'2'] };
                        for (name, value) in [
                        ...
                        let bytes = if stage == 1 && defect == "truncated" {
                ...
                if defect == "guard" {
                    assert_eq!(outcome, Err(SubmissionSendRefusal::Identity));
                } else if defect == "token" {
        ...
        if accepted {
            ...
            for mode in [0, 1, 2, 3, 4] {
                ...
                let refused = if mode == 4 {
                    ...
                    transport
                        .lookup_physical_snapshot(&moved, &submission, "job /?é", &authentication)
                        .await
                };
                assert!(refused.is_err());
                let zero_generation = if mode == 4 {
                    transport
                    ...
                    transport
                        .lookup_physical_job(&identity, &submission, "job /?é", 0, &authentication)
                        .await
                };
                ...
                for (status, defect) in [
                    ...
                    body["physical_sling_job_identifiers"] =
                        serde_json::json!([if defect == "physical" {
                            "other-job"
                        } else {
                            "job /?é"
                        }]);
                    ...
                    let peer = async {
                        ...
                        let payload = if defect.ends_with("truncated") {
                            &body[..body.len() - 1]
                        } else {
                            &body
                        };
...
async fn selected_artifact_stream_requires_complete_verified_message() {
    ...
    for mode in 0..6 {
        ...
        let endpoint = format!(
            "{}://{}/aem",
            if mode == 1 || mode == 2 { "https" } else { "http" },
            listener.local_addr().unwrap()
        );
        let mut files = profile_files();
        replace_profile(&mut files, "profiles/mike.toml", |text| {
        ...
        for (status, defect, accepted) in [
            ...
            let peer = async {
                ...
                let mut stream: Box<dyn Peer> = if mode == 1 || mode == 2 {
                    let version =
                        if mode == 1 { &rustls::version::TLS12 } else { &rustls::version::TLS13 };
                    let mut config = rustls::ServerConfig::builder_with_provider(Arc::new(
                        rustls::crypto::ring::default_provider(),
                    ))
                    .with_protocol_versions(&[version])
                ...
                if mode >= 3 {
                    ...
                    authentication.lend_value_bytes(|value| {
                        assert!(request.windows(value.len()).any(|part| part == value))
                    });
                ...
                if defect == "trailer" {
                    wire.extend_from_slice(&encode(1, 5, &[]));
                }
