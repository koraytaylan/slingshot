...
fn replace_profile(
    ...
    let old: String = Sha256::digest(&*profile).iter().map(|byte| format!("{byte:02x}")).collect();
    *profile = change(std::str::from_utf8(profile).expect("profile text")).into_bytes();
    let new: String = Sha256::digest(&*profile).iter().map(|byte| format!("{byte:02x}")).collect();
...
#[path = "environment_provider/selection.rs"]
mod selection;

#[path = "environment_provider/events.rs"]
mod events;

#[path = "environment_provider/high_water.rs"]
mod high_water;

#[path = "environment_provider/connection.rs"]
mod connection;

#[path = "environment_provider/refresh.rs"]
mod refresh;

#[path = "environment_provider/finite.rs"]
mod finite;

#[path = "environment_provider/submission.rs"]
mod submission;

#[path = "environment_provider/artifacts.rs"]
mod artifacts;
